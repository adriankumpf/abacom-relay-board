...
pub fn set_output(handle: &libusb::DeviceHandle<'_>, data: u8) -> Result {
    let msg = vec![
...
pub fn get_input(handle: &libusb::DeviceHandle<'_>) -> Result<Vec<u8>> {
    let msg = vec![0xA0];
    write(handle, msg)?;
    read(handle)
}

fn write(handle: &libusb::DeviceHandle<'_>, mut data: Vec<u8>) -> Result {
...
fn read(handle: &libusb::DeviceHandle<'_>) -> Result<Vec<u8>> {
