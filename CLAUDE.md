...
- **Mutation-check new protocol logic** — a passing suite that catches nothing is worse than none. Break it deliberately and confirm the failure before claiming coverage: an LSB-first `shift_out_bits`, an LSB-first register read, a dropped register restore and a sample taken after the rising edge are all caught today, as are the four error-path guarantees (a restore skipped when a transfer fails, a lost read or a failed restore reported as a plain transport error, and a failed restore letting the error before it win). The `Error::RegisterOutOfSync` source chain is pinned too, because thiserror wires `Error::source()` from the field *name* and a rename would silently drop it
