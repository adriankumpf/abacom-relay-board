...
1. **Public API** (`src/lib.rs`) — `Usb` is a libusb context and finds boards; `usb.board(port)` returns a `Board`, which has `relays()`, `set_relays()`, `self_test()`, `reset_device()`, `port()` and `location()`. The optional USB port number disambiguates multiple boards, `usb.boards()` enumerates them all, and `usb.board_at(location)` names one back from a stored `Location`.
...
- **Accessors on `Location` (`bus()`, `hops()`, a public `port()`).** `bus()` and `port()` were public for one commit and came back out; the surface is deliberately `Clone + Debug + Eq + Ord + Hash + Display + FromStr` and nothing else. Half-exposure is worse than none: `hops` has no accessor, so `bus()`/`port()` let a caller read the first component and the last but not the middle — `1-1.2.3` and `1-1.9.3` are indistinguishable through them — and with no public constructor nothing can be rebuilt from what was read, so the round trip runs through `Display`/`FromStr` regardless. `port()`'s `Option` was the tell: `FromStr` rejects the hopless case and `Location::of` is reached only through `is_ch341a`, which no root hub passes, so the `None` cannot occur and a public signature would only teach callers to `.expect()`. The fields are private, so adding an accessor later is a minor change, while removing one after v0.8.0 tags costs a coordinated arb-ex release — nothing has asked for the parts yet
