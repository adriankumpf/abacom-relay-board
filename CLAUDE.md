...
- `Board::relays()` is a plain read; `Board::self_test()` writes an inverted test pattern to the shift register (without latching), verifies the read-back and restores the register. It is a *diagnostic*, not a guard on the operating path: `set_relays` with `Verify::Enabled` already covers the same ground inside one claim and on the value the caller asked for, and a `self_test` is its own claim so it vouches for no read but its own. Don't recommend it as a periodic correctness check — as a health probe it beats a periodic read only because it knows the answer it expects. It returns the `Relays` it read on its way past, which it has to read before it can write anything: that is what `arb --status` prints, so the state printed and the check that vouched for it come from one claim
- Reading the register is destructive, so **every read restores it on every path out**, failures included: `A6275::restoring` is the combinator that guarantees it, and a bail-out before the restore leaves the register holding zeros while the relays stay energized, so the *next* read lies without any relay having moved. Where the register is left disagreeing with the outputs the caller gets `Error::RegisterOutOfSync` instead of a transport error, because the documented remedy for a transient fault is to retry and a retried read would succeed and report "none active". Both halves produce it: a read that failed (nothing to put back) and a restore that failed (something to put back, and it did not land). `Board` holds no state between calls, so telling the caller at the moment of failure is the only lever there is. Note the variant is reachable from `set_relays` too, where the latch has already happened — the error means the register is lost, not that no relay moved. Also why a failed verification restores the value that was *latched*, not the read-back: a mismatch implicates the read path, and reading a board back cannot settle what actually happened, since the A6275 hands back its shift register and not its outputs
...
- **Mutation-check new protocol logic** — a passing suite that catches nothing is worse than none. Break it deliberately and confirm the failure before claiming coverage: an LSB-first `shift_out_bits`, an LSB-first register read, a dropped register restore and a sample taken after the rising edge are all caught today, as are the six error-path guarantees (a restore skipped when a transfer fails, a verification restoring the read-back instead of the latched value, a lost read or a failed restore reported as a plain transport error, a failed restore letting the error before it win, and `self_test` returning anything other than what it found). The `Error::RegisterOutOfSync` source chain is pinned too, because thiserror wires `Error::source()` from the field *name* and a rename would silently drop it
- **The error paths are testable too, and are tested.** `FailingAfter` in `src/a6275.rs` (built by the `failing()` helper, beside `fake()` and `flaky()`) fails a transfer part way through, with the read failing *after* the simulated register has been clocked out, which is what the wire does. Anything new on a failure path wants a case there rather than a comment claiming it is fine
